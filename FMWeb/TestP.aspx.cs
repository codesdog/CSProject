...
namespace FMWeb
{
    public partial class TestP : System.Web.UI.Page
    {
        ...
        {
            btnCluster.Attributes.Add("onclick", "javascript:document.getElementById('spErr').innerHtml='正在处理中，请稍后...'");
        }
        #region Cluster
        /// <summary>
        /// 返回一段语料中的语族
        ...
        protected void btnCluster_Click(object sender, EventArgs e)
        {
            ...
            lbT1.Text = "文本处理时间："+ExecDateDiff(dt1, DateTime.Now);
            dt1 = DateTime.Now;
            int intlength = 2;
            DataTable dt = GetClusterFromCorpus(str, intlength);
            lbT2.Text ="计算时间："+ ExecDateDiff(dt1, DateTime.Now);
            dt1 = DateTime.Now;
            gv1.DataSource = dt;
            gv1.DataBind();
            lbT3.Text = "绑定输出时间：" + ExecDateDiff(dt1, DateTime.Now);
        }
        #region 返回时间差
